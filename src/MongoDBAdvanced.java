...
public class MongoDBAdvanced {
	...
	private static void selectSingleRecordAndFieldByRecordNumber(DBCollection yourDBCollection) {
		...
		DBCursor cursor = yourDBCollection.find(whereQuery, fields); // Select document with employeeID 3 and print only
																		// that field. Keep 1 always in fields.
	...
	private static void regex_Example(DBCollection yourDBCollection) {
		...
		regexQuery.put("employeeName", new BasicDBObject("$regex", "TestEmployee_[3]").append("$options", "i"));
